...
| 30b | ~~The SDK's native tool executor is disabled at adapter construction~~ → **the SDK is never given an executable body**: tools are declared through `ExternalToolset`, whose `call_tool` raises unconditionally (`toolsets/external.py:44`) | *Reworded 2026-08-27 after spike 06.* There is no executor to disable, so "disabled at construction" described a mechanism we do not use. Give the SDK a real function ⇒ two execution paths, one ungoverned — **VERIFIED, spike 06 gate 2 + its `FunctionToolset` control** |
| 30c | An adapter that cannot route a tool through the ledger **cannot register that tool** | allow registration ⇒ an effect path the ledger never sees. *Vendor-hosted tools remain unsupported in v0.1 (retracted 2026-08-27); spike 06 did not admit them — `ExternalToolset` cannot express them, because there is no local body to withhold.* **VERIFIED — spike 06 gate 6** (`UninterceptableTool` raised at registration) |
...
| 30e | A dispatched effect whose body **raised** settles `indeterminate`, never `intended` | leave it `intended` ⇒ indistinguishable from an effect that never started, so a crashed charge looks like no charge. **VERIFIED — spike 06 gate 11**, which found this defect in our own harness |
