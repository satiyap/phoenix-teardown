...
| 4 | **Adapter contract + one SDK adapter** | We build and operate the agents on **one internal harness, on Pydantic AI 2.35.0** (amended 2026-08-27, superseding the two-SDK plan; five reasons in ADR-0004's evidence log). `sdk_subprocess` (renamed from `sdk_in_process` 2026-08-29; the driver spawns the harness and passes it a socketpair) is the only mode shipped; tools are declared through `ExternalToolset` so **the SDK never receives an executable body** *(corrected 2026-08-27 by spike 06 — the earlier wording said the native tool executor "is disabled", which described a mechanism we do not use)*, and every tool call crosses the typed `ToolCall → ledger → ToolResult` boundary. **Verified: 57 gate assertions** *(amended 2026-08-28, superseding "53", which superseded "26": round 5 added the intent/dispatch identity check, the duplicate-registration refusal, the primary-key-across-runs gate and the auto-injected-wrapper gate; round 3 added the structural-refusal gate 13 and the spec-conformant ledger gate 12, and round 4 added the argument-binding, fence-token, duplicate-intent, `capabilities=` and pin-frontier assertions)*, including the resume path (`_tool_execution.py:399`) and every vendor-hosted tool the SDK ships. The adapter contract is kept so SDKs stay swappable. | ADR-0004 (amended 2026-08-27), ADR-0006; AX contract, Omnigent integration modes |
...
| 3 | **No model gateway or provider abstraction.** **Amended 2026-09-03 (OQ-147): this prohibits *building* one, not *integrating* one.** A provider router may be embedded **as a library inside the egress proxy** — never as a gateway deployed beside it — because the proxy is already a Phoenix component holding and injecting the model-provider key ([`../spec/14-credentials.md`](../spec/14-credentials.md), OQ-075) and `CreateSpec.EgressVia` already admits exactly one destination. Three invariants survive or the integration does not happen: the harness stays the sole usage source (OQ-149), the router authors no policy (OQ-150), and it opens no second route off the pod ([`../spec/15-sandbox.md`](../spec/15-sandbox.md) §5). | Explicit anti-goal of this study, and a commodity: MAF ships 35 provider packages, which is exactly the surface we should not own. The amendment does not weaken that: owning 35 provider packages and calling a library that owns them are different liabilities, and the boundary's own INTEGRATE test — permissive licence, our language, independently useful, replaces a subsystem we would otherwise invent — had never been applied to a router. |
...
>
> **Amended 2026-09-03 (OQ-147):** *build* is load-bearing in that sentence. A model router
> **integrated as a library inside the egress proxy** is admitted by item 3 as amended; a model
> gateway deployed as its own service is not, because it would take a second route off the pod
> and stand up a second place where policy is authored.
