...
Answer: Four things, each because the evidence shows no adequate precedent. **(1) Content-derived version pinning applied to the agent definition** — MAF proves the mechanism on workflows (a bytecode digest enforced on restore) and nobody applies it to agents; this is the clearest differentiator in the study. **(2) The effect ledger** — two partial precedents with different keys (Cloudflare's explicit `idempotency_key`, AG2's `causation_id`) and nobody makes it platform-owned; ADK explicitly delegates it to tool authors. **(3) The unified `Capability` + `Extension` model with a two-layer conformance bench** — Omnigent has the bench, Pydantic AI has the composition, nobody has both. **(4) Agent-level revocation** — thirteen projects, and the best available is credential-level (AgentCore) or attachment-level (Agent Control).
Evidence: ADR-0011 amendment 3; ADR-0014; ADR-0012 amendment 3; `synthesis/build-reuse-map.md`; D9 negative in 10 of 13, `implicit` in 3

### Q23. What do we reuse or integrate?
Answer: Two INTEGRATE candidates and a short list of ports. **INTEGRATE: AG2's `ag2.network`** — Apache-2.0, Python, opt-in, tested; the envelope schema and hub contract solve the one subsystem we would otherwise invent with no prior art. **INTEGRATE: Agent Control** — Apache-2.0, Python, self-hostable, and the only project built on the assumption that it is *not* the whole platform (adapters for four runtimes it does not own). **INTEGRATE: Cedar** for policy, and `genai-prices` for cost data. **PORT: Pydantic AI's `_ssrf.py`** guard list, Omnigent's secretless credential proxy, AX's fold-the-log state derivation and composite-PK single-writer, Cloudflare's hung-work detection, HumanLayer's `Approval` schema, ADK's in-memory-sibling discipline.
Evidence: `synthesis/build-reuse-map.md`; ADR-0003; ADR-0013 amendment 1; Q6 verdicts across all 13 facts.yaml
...
Answer: Five hard boundaries, each backed by evidence rather than preference. **(1) No compensation or rollback engine** — zero positive answers in thirteen projects, including the one whose whole job is control and the one whose whole job is orchestration; Pydantic AI shows why: saga compensation belongs to a durable workflow engine, so we integrate Temporal/DBOS/Prefect rather than reimplement it. **(2) No workflow engine of our own** — MAF and Pydantic AI both demonstrate that orchestration is a separable, better-solved concern. **(3) No model gateway or provider abstraction** — an explicit anti-goal of this study, and a solved commodity. **(4) No agent-authoring framework** — we adapt agents, we do not compete with the frameworks that write them; Cloudflare's coherent alternative (write the agent against our runtime) is rejected only because adapting existing agents is a hard requirement. **(5) No bespoke policy DSL, trace format, or messaging protocol** — adopt Cedar, OTel and AG2's envelope instead.
