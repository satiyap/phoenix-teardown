...
| **Process boundary (OQ-056, 2026-08-28)** | Go control plane + **Python harness sidecar** in the same pod, §07 frames over a Unix socket (`0600` auth). `sdk_in_process` keeps its value, redefined. mTLS/remote half still unshipped | three Python-by-evidence components all sit in the data plane; every Go advantage is control plane; the pod is already the disposable unit | `spec/07` §Transport; `spec/01` comment; `DESIGN.md` §7a; ADR-0004 `confirms`; ADR-0016 |
| **Container layout (spike 05 T2, 2026-08-29)** | One container: Go driver is PID 1 and spawns the Python harness; socket in a driver-created `0700` dir. `sdk_sidecar` → `sdk_subprocess` | gVisor does not propagate a UDS inode across containers on a shared volume (bisected); loopback/abstract sockets would discard `0600` | ADR-0016:63 amended; ADR-0009 evidence; `spec/01`, `spec/07`, OpenAPI, `DESIGN` §7a |
