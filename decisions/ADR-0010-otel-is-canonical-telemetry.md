...
| AWS AgentCore | neutral | `src/bedrock_agentcore/_utils/user_agent.py @ 826416a` | No OTel in the SDK; telemetry is CloudWatch and X-Ray server-side. One good minor idea worth noting: `build_user_agent_suffix(integration_source)` attributes calls to a client library, which is how a managed service deprecates a client version with data rather than by announcement. |
| Microsoft Agent Framework | neutral | `python/packages/core/agent_framework/_telemetry.py @ e34bf48` | `_telemetry.py`, `_evaluation.py` and `devui` exist; semantic-convention adherence was not audited in this targeted pass, so recorded `unknown` rather than guessed (OQ-038). |
| Agent Control | confirms | `models/src/agent_control_models/observability.py:30-460 @ 7cb21af` | Extends the amendment. Decisions are not log lines but a **modelled, queryable, aggregable dataset**: `ControlExecutionEvent`, `BatchEventsRequest`, `EventQueryRequest`, `ControlStats`, `TimeseriesBucket`, plus a dashboard. AG2 taught that refusals must be *observable*; this adds that they must be **aggregable** — otherwise observe mode produces data nobody can act on. |
