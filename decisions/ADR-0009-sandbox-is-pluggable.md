...
| **Spike 05 T2 (2026-08-29)** | amends | `spikes/05-*/T2-FINDING.md` @ `9f21e88` | Provider behaviour differs from plain containers in a way the design must know: under gVisor, a UDS on a shared pod volume is invisible to a second container. Isolation guarantees *and* IPC semantics vary by provider — surface both to the placement layer (ADR-0016), not only the security properties this ADR listed. |
