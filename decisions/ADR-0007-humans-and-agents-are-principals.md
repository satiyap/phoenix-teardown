...
| Agent Control | challenges | `models/src/agent_control_models/ @ 7cb21af` | **Fourth project with exactly half the model, and the pattern is now unmistakable.** Agent Control governs *behaviour* thoroughly — runtime-configurable controls, recursive conditions, three-way outcomes, full decision audit — and cannot express *authority* at all: no principals, no end users, no on-whose-behalf, no record of whom a decision concerned. With ADK (credentials, no principals), HumanLayer (decisions, no approver) and AgentCore (both halves), **the principal model is the field's most consistent omission**, and it is precisely what ties policy, approval and delegation together. |
